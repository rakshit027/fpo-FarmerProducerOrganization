...
$searchqueryparameter=$_GET["id"];
  ...
  $currentdate=strftime("%B-%d-%Y %H-%M-%S",$currenttime);//this will take 2 Arguments 1-DATE FORMATE and 2-Current time variable
    ...
    $_SESSION["Errormsg"]="Post description should be less than 10000 characers";
    redirect_to("posts.php");
  }else {
    ...
    if (!empty($_FILES["images1"]["name"])) {
      // code...
      $sql = "UPDATE posts SET title='$posttitle',category='$category',image1='$Images1',post='$postText'
      WHERE id='$searchqueryparameter' ";

    }elseif (!empty($_FILES["images2"]["name"])) {
      $sql = "UPDATE posts SET title='$posttitle',category='$category',image2='$Images2',post='$postText'
      WHERE id='$searchqueryparameter' ";

    }elseif(!empty($_FILES["images1"]["name"])||!empty($_FILES["images2"]["name"])) {
      $sql = "UPDATE posts SET title='$posttitle',category='$category',image1='$Images1',image2='$Images2',post='$postText'
      WHERE id='$searchqueryparameter' ";
...
<header class="bg-dark text-white py-3 "><!--here padding is used ON ""Y-axis""-->
  <div class="container">
    <div class="row">
      <div class="col-md-12">
      <h3><i class="fas fa-edit" style="color:yellow;"></i> Edit Your post </h3>
      </div>
    </div>
  </div>
</header>
