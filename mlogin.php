...
if (isset($_SESSION["muser_id"])) {
  ...
  if (isset($_POST["Submit"])) {
    ...
    if (empty($username)||empty($password)) {
      ...
      redirect_to("mlogin.php");
    }else {
      $account_found=mlogin_attempt($username,$password);
      if ($account_found) {
        // code...
        $_SESSION["muser_id"]=$account_found["id"];
        $_SESSION["Username"]=$account_found["Username"];
    //    $_SESSION["AdminName"]=$account_found["aname"];
        $_SESSION["Successmsg"]="WELCOME Member :) ".$_SESSION["Username"];
// if our tracking URL is set it will redirect to URL we etered
        if (isset($_SESSION["trackingURL"])) {
          // code...
          //redirect_to($_SESSION["trackingURL"]);
        }else {
          redirect_to("formembers.php?page=1");
        }
      }else {
        $_SESSION["Errormsg"]="incorrect UserName/password Or Not Approved By Org.";
        redirect_to("mlogin.php");
      }
      ...
      <div class="offset-sm-3 col-sm-6" style="min-height:650px;">
        ...
        <div class="card bg-secondary text-light">
          <div class="card-header">
            <h4>WELCOME back !!! To Your FPO</h4>
            </div>
            <div class="card-body bg-dark">
            <form class="" action="mlogin.php" method="post">
              <div class="form-group">
                <label for="username"><span class="FieldInfo">Member UserName :</span></label>
